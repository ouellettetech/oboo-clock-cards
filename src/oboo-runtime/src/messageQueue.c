...
int _connectMessageQueue(int clientType, const char* host, int port, const char* clientId, int bTls, const char* willTopic, const char* willPayload) {
	int status = 0;
	...
	char newClientID[CLIENT_ID_LEN+1];
	if (strlcpy(newClientID, clientId, sizeof(CLIENT_ID_LEN)) >= sizeof(CLIENT_ID_LEN))
         return -1;

    mosqPtrs[clientType] = mosquitto_new(clientId, true, newClientID);
	...
	if (willTopic != NULL && willPayload != NULL) {
		status = mosquitto_will_set( mosqPtrs[clientType],
									willTopic,		// topic
									(int)strlen(willPayload),				// payloadlen
									willPayload,	// pauload
									MQTT_QOS, // qos
...
int _connectMessageQueueSequence(int clientType, const char* host, int port, const char* clientId, int bTls, const char* willTopic, const char* willPayload) {
	int count = 0;
	int status = 1;
