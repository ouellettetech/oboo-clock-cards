...
#include <pthread.h>
static struct ThreadInf aTi[MAX_NUMBER_OF_THREADS];

static int curRun = 0; //currently running
static int tIndex[MAX_NUMBER_OF_THREADS];
...
void getExeDir (char *path) {
  ...
  numChars = (int)readlink("/proc/self/exe", buf, MAX_CHAR_LEN);
...
void invokeCallback(char* cbIndex) {
  ...
  printf("sizeof calloc %d\n", 7+(int)sizeof(cbIndex)*(int)sizeof(char));
  char * buff = malloc(7+sizeof(cbIndex)*sizeof(char));
  memset(buff, 0, 7+sizeof(cbIndex)*sizeof(char));
  sprintf(buff, "%s_%s", CB_PREFIX, cbIndex);//TODO: removing the \0 since it shouldn't be inside a string. not sure why it was here...
...
duk_ret_t native_print(duk_context *ctx) {
  const char *initString = (const char*) js_get_string(ctx, 0);
...
int httpRequest (const char* url, const char* method, char* response) {
  CURL *curl;
  CURLcode res;
...
duk_ret_t native_httpRequest(duk_context *ctx) {
  ...
  duk_get_prop(ctx, 0);
  const char * url = (const char*) duk_to_string(ctx, -1);
  printf("url: %s\n", url);

  duk_push_string(ctx, "method");
  duk_get_prop(ctx, 0);
  const char * method = (const char*) duk_to_string(ctx, -1);
  printf("method: %s\n", method);
...
duk_ret_t native_setButton(duk_context *ctx) {
  ...
  return status;  /* no return value (= undefined) */
}
...
duk_ret_t native_serialRead(duk_context *ctx) {
  ...
  duk_push_string(ctx, buf);
  return status; /* 1 = return value at top */
}
...
duk_ret_t native_writeFile(duk_context *ctx) {
  int status = 0;
  const char* filePath  = (const char*) duk_to_string(ctx, 0);
  const char* content   = (const char*) duk_to_string(ctx, 1);
