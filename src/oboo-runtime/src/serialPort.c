...
// global vars
static int serialFd = FD_DEFAULT;
...
# if ORT_SERIAL_PORT_ENABLED == 1
    ...
    set_interface_attribs(serialFd, (int)baud);
...
int writeSerialPort(const char* msg) {
    int wlen;
    int expLen = (int) strlen(msg);

    if (serialFd != FD_DEFAULT) {
        // printf("  writeSerialPort writing message of length %d\n", expLen);
        wlen = (int)write(serialFd, msg, expLen);
        if (wlen != expLen) {
...
int writeCharSerialPort(unsigned char c) {
    ...
    if (serialFd != FD_DEFAULT) {
        wlen = (int)write(serialFd, &c, expLen);
        if (wlen != expLen) {
            printf("Error from write: %d, %d\n", wlen, errno);
            return -1;
        }
...
int readSerialPort(char *msg) {
    ...
    rdlen = (int)read(serialFd, buf, sizeof(buf) - 1);
    if (rdlen > 0) {
        buf[rdlen] = 0;
        printf("Read %d: \"%s\"\n", rdlen, buf);
